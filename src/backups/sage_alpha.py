# This file was *autogenerated* from the file sage_alpha.sage.
from sage.all_cmdline import *   # import sage library
_sage_const_3 = Integer(3); _sage_const_2 = Integer(2); _sage_const_1 = Integer(1); _sage_const_0 = Integer(0); _sage_const_6 = Integer(6); _sage_const_4 = Integer(4); _sage_const_40 = Integer(40); _sage_const_15 = Integer(15); _sage_const_4p2 = RealNumber('4.2'); _sage_const_30 = Integer(30)
r"""
...
# sample functions
myfunc0 = _sage_const_4p2 
myfunc1 = _sage_const_1 /_sage_const_4 *(x - _sage_const_4 )*(pi + _sage_const_4 *x)*x
myfunc2 = (_sage_const_2 *x**_sage_const_2  + _sage_const_3 *x + _sage_const_1 )/x
myfunc3 = arctan(_sage_const_3 *x) + (_sage_const_6  - pi/_sage_const_2 )
myfunc4 = _sage_const_1 /(x-_sage_const_2 ) + _sage_const_3 
...
def stationary_points(self):
    ...
    if not self.is_real():
        x = self.variables()[_sage_const_0 ]
    else:
        x = var('x')
    assume(x, 'real')
    __tmp__=var("x"); fnct = symbolic_expression(self).function(x)

    x_coords = solve([fnct.diff(_sage_const_1 ) == _sage_const_0 ], x, to_poly_solve="force")
    stats = [(pt.right(), fnct(pt.right())) for pt in x_coords if pt.left() == x
            and pt.right() in RR]
    stats.sort()
    return stats
...
def inflection_points(self):
    ...
    x_coords = solve([fnct.diff(_sage_const_2 ) == _sage_const_0 ], x, to_poly_solve="force")
...
def asymptotes(self):
    ...
    c = limit(fnct, x = -oo)
    if c.is_real():
        __tmp__=var("x"); asy = symbolic_expression(c).function(x)
        results.append(asy)
    ...
    if a.is_real() and b.is_real():
        __tmp__=var("x"); asy = symbolic_expression(a*x + b).function(x)
        results.append(asy)
...
def monotonicity(self, type="iso", pure=False):
    ...
    TESTS:
        ...
        sage: monotonicity(myfunc0)
        [(-Infinity, +Infinity)]

    ::

        sage: monotonicity(myfunc1)
        [[x <= -1/12*pi - 1/12*sqrt(16*pi + pi^2 + 256) + 4/3], [x >= -1/12*pi
        + 1/12*sqrt(16*pi + pi^2 + 256) + 4/3]]
    ...
    end_pts = [-oo]
    end_pts.extend([pt[_sage_const_0 ] for pt in stationary_points(self)])
    end_pts.append(oo)

    if len(end_pts) < _sage_const_3 :                        # constant function
        ...
        # investigated intervals of monotonicity
        ints = [ (end_pts[i-_sage_const_1 ],end_pts[i]) for i in range(_sage_const_1 ,len(end_pts))]

        ## representants of investigated intervals of monotonicity
        test_pts = [end_pts[_sage_const_1 ] - _sage_const_1 ]
        test_pts.extend( [(it[_sage_const_0 ]+it[_sage_const_1 ])/_sage_const_2  for it in ints[_sage_const_1 :-_sage_const_1 ]] )
        test_pts.append(end_pts[-_sage_const_2 ] + _sage_const_1 )

        if type == "iso":
            if pure:
                return [ints[i] for i in range(len(ints))
                        if self.diff(_sage_const_1 )(test_pts[i]) > _sage_const_0 ]
            else:
                return [ints[i] for i in range(len(ints))
                        if self.diff(_sage_const_1 )(test_pts[i]) >= _sage_const_0 ]
        elif type == "anti":
...
def curvature(self, type="convex", pure=False):
    ...
    if type == "convex":
        if pure:
            return solve_ineq(self.diff(_sage_const_2 ) > _sage_const_0 )
        else:
            return solve_ineq(self.diff(_sage_const_2 ) >= _sage_const_0 )
    elif type == "concave":
        if pure:
...
def investigate(self, left=-_sage_const_2 *pi, right=_sage_const_2 *pi, down=-_sage_const_15 , up=_sage_const_15 ):
    r"""
    ...
    stat_pts = point(stats, rgbcolor='orange', pointsize=_sage_const_30 )
    pic = stat_pts
    ...
    infl_pts = point(infls, rgbcolor='green', pointsize=_sage_const_40 )
    pic += infl_pts
